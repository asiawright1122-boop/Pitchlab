...
export default function BettingMarkets({
  ...
  return (
    <div className="bg-white border border-gray-200/80 rounded-3xl p-4 sm:p-5 shadow-[0_8px_32px_rgba(0,0,0,0.03)] transition-all relative overflow-hidden select-none text-[#1c1c1e]">
      
      {/* 🧭 Tab Switcher */}
      <div className="flex border-b border-gray-200/80 pb-2 mb-5 overflow-x-auto gap-2.5 custom-scrollbar min-w-0">
        {[
          ...
          <button
            ...
            className={`pb-2.5 px-2 font-black text-[10px] uppercase tracking-widest transition-all relative whitespace-nowrap ${
              activeTab === tab.id
      ...
      {activeTab === "popular" && (
        <div className="flex flex-col gap-6">
          ...
          {probHome > 0 && (
            <div className="bg-gray-50/50 border border-gray-150 rounded-2xl p-4 relative overflow-hidden flex flex-col gap-3.5">
              
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-[8px] font-black px-2 py-0.5 bg-[#34c759]/10 text-[#248a3d] rounded border border-[#34c759]/20 uppercase tracking-wider">
                    Dixon-Coles Model
                  </span>
                  <h3 className="text-[10px] font-black text-gray-700 uppercase tracking-widest">Model Forecast</h3>
                </div>
                <div className="flex items-center gap-1 bg-white px-2 py-0.5 rounded border border-gray-200">
                  ...
                  <span className="text-[7.5px] font-black text-gray-400 tracking-wider">QUANT LIVE</span>
                </div>
              </div>
              ...
              <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden flex border border-gray-200/50 shadow-inner">
                <div style={{ width: `${probHome * 100}%` }} className="bg-[#34c759] h-full relative" />
                <div style={{ width: `${probDraw * 100}%` }} className="bg-gray-300 h-full relative" />
                <div style={{ width: `${probAway * 100}%` }} className="bg-[#007aff] h-full relative" />
              </div>

              {/* Detailed Odds Grid */}
              <div className="grid grid-cols-3 gap-2.5 text-center">
                {/* Home */}
                <div className="bg-white border border-gray-150 rounded-xl p-2 flex flex-col items-center justify-between min-w-0">
                  <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest truncate w-full">{homeTeam.substring(0, 3)} Win</span>
                  <span className="text-xs font-black text-gray-900 mt-0.5">{(probHome * 100).toFixed(0)}%</span>
                  <div className="text-[7px] text-gray-400 mt-1 flex flex-col leading-tight">
                    <span>Fair: {fairHome.toFixed(2)}</span>
                    <span className="text-gray-600 font-bold mt-0.5">Mkt: {homeOdds > 0 ? homeOdds.toFixed(2) : "-"}</span>
                  </div>
                  {edgeHome > 0 ? (
                    <span className="mt-2 text-[7px] font-black text-[#248a3d] bg-[#34c759]/10 px-1 py-0.5 rounded border border-[#34c759]/20 truncate w-full max-w-full text-center">
                      +{edgeHome.toFixed(0)}% Edge
                    </span>
                  ) : (
                    <span className="mt-2 text-[7px] font-bold text-gray-400 bg-gray-100 px-1 py-0.5 rounded truncate w-full max-w-full text-center">
                      No Value
                    </span>
                  )}
                </div>
                ...
                {/* Draw */}
                <div className="bg-white border border-gray-150 rounded-xl p-2 flex flex-col items-center justify-between min-w-0">
                  <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest truncate w-full">Draw</span>
                  <span className="text-xs font-black text-gray-900 mt-0.5">{(probDraw * 100).toFixed(0)}%</span>
                  <div className="text-[7px] text-gray-400 mt-1 flex flex-col leading-tight">
                    <span>Fair: {fairDraw.toFixed(2)}</span>
                    <span className="text-gray-600 font-bold mt-0.5">Mkt: {drawOdds > 0 ? drawOdds.toFixed(2) : "-"}</span>
                  </div>
                  {edgeDraw > 0 ? (
                    <span className="mt-2 text-[7px] font-black text-[#248a3d] bg-[#34c759]/10 px-1 py-0.5 rounded border border-[#34c759]/20 truncate w-full max-w-full text-center">
                ...
                {/* Away */}
                <div className="bg-white border border-gray-150 rounded-xl p-2 flex flex-col items-center justify-between min-w-0">
                  <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest truncate w-full">{awayTeam.substring(0, 3)} Win</span>
                  <span className="text-xs font-black text-gray-900 mt-0.5">{(probAway * 100).toFixed(0)}%</span>
                  <div className="text-[7px] text-gray-400 mt-1 flex flex-col leading-tight">
                    <span>Fair: {fairAway.toFixed(2)}</span>
                    <span className="text-gray-600 font-bold mt-0.5">Mkt: {awayOdds > 0 ? awayOdds.toFixed(2) : "-"}</span>
                  </div>
                  {edgeAway > 0 ? (
                    <span className="mt-2 text-[7px] font-black text-[#248a3d] bg-[#34c759]/10 px-1 py-0.5 rounded border border-[#34c759]/20 truncate w-full max-w-full text-center">
          ...
          <div className="border border-gray-150 bg-gray-50/20 rounded-2xl p-4">
            <h3 className="text-[10px] font-black text-gray-700 uppercase tracking-widest mb-4 flex items-center gap-1.5">
              <span className="w-1.5 h-3.5 bg-[#34c759] rounded-full shadow-[0_0_8px_rgba(52,199,89,0.2)]"></span> 
              Match Winner (1X2)
            </h3>
            {homeOdds > 0 ? (
              <div className="grid grid-cols-3 gap-2.5">
                ...
                <button
                  ...
                  className={`py-3 px-1 rounded-xl border transition-all duration-300 flex flex-col justify-center items-center gap-1.5 min-w-0 ${
                    isSelected("1x2-h")
                      ? "border-[#34c759] bg-[#34c759]/10 text-gray-900 shadow-[0_0_12px_rgba(52,199,89,0.1)]"
                      : edgeHome > 0 
                        ? "border-[#34c759]/25 bg-white hover:border-[#34c759]/40 text-gray-800" 
                        : "border-gray-200 bg-white hover:border-[#34c759]/40 text-gray-700"
                  }`}
                >
                  <span className="text-[8.5px] text-gray-400 font-black uppercase tracking-widest truncate w-full text-center">{homeTeam.substring(0, 3)}</span>
                  <span className="text-sm font-black leading-none">{homeOdds.toFixed(2)}</span>
                ...
                >
                  <span className="text-[8.5px] text-gray-400 font-black uppercase tracking-widest text-center">DRAW</span>
                  <span className="text-sm font-black leading-none">{drawOdds.toFixed(2)}</span>
                ...
                >
                  <span className="text-[8.5px] text-gray-400 font-black uppercase tracking-widest truncate w-full text-center">{awayTeam.substring(0, 3)}</span>
                  <span className="text-sm font-black leading-none">{awayOdds.toFixed(2)}</span>
                  {renderSparkline(getSparklinePoints("1x2", "away"), edgeAway > -10)}
                </button>
              </div>
            ) : (
              <p className="text-[9px] text-gray-400 text-center py-2 uppercase font-black tracking-widest">No odds available</p>
            )}
          </div>
                ...
                {ouSelections
                  ...
                  .map((o) => (
                    ...
                    >
                      ...
                      <span className="text-sm font-black">{o.price.toFixed(2)}</span>
                    </button>
                  ))}
              </div>
            ) : (
              <p className="text-[9px] text-gray-400 text-center py-2 uppercase font-black tracking-widest">No O/U odds available</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="border border-gray-150 bg-gray-50/20 rounded-2xl p-4">
            ...
            {spreads.length > 0 ? (
                    ...
                    <div key={`spread-row-${spreadVal}`} className="grid grid-cols-2 gap-3">
                      {homeItem ? (
                        <button
                          ...
                          className={`py-3 px-3 rounded-xl border transition-all duration-300 flex justify-between items-center text-xs ${
                            isSelected(homeItem.key)
                              ? "border-[#34c759] bg-[#34c759]/10 text-gray-900 shadow-[0_0_12px_rgba(52,199,89,0.1)]"
                              : "border-gray-200 bg-white hover:border-[#34c759]/40 text-gray-700"
                          }`}
                        >
                          <span className="font-bold truncate max-w-[70%]">{homeTeam.substring(0, 3)} {homeItem.spread > 0 ? `+${homeItem.spread}` : homeItem.spread}</span>
                          <span className="font-black text-[#248a3d]">{homeItem.price.toFixed(2)}</span>
                        </button>
                      ) : <div />}
                      {awayItem ? (
                        <button
                        ...
                        >
                          <span className="font-bold truncate max-w-[70%]">{awayTeam.substring(0, 3)} {awayItem.spread > 0 ? `+${awayItem.spread}` : awayItem.spread}</span>
                          <span className="font-black text-[#248a3d]">{awayItem.price.toFixed(2)}</span>
                        </button>
                      ) : <div />}
                    </div>
                  );
              ...
              <p className="text-[9px] text-gray-400 text-center py-2 uppercase font-black tracking-widest">No handicap odds available</p>
            )}
          </div>
                  ...
                  return (
                    <div key={`ou-row-${thresholdVal}`} className="grid grid-cols-2 gap-3">
                      {overItem ? (
                        ...
                        >
                          <span className="font-bold truncate">Over {thresholdVal}</span>
                          <span className="font-black text-[#248a3d]">{overItem.price.toFixed(2)}</span>
                        </button>
                      ) : <div />}
                      {underItem ? (
                        <button
                        ...
                        >
                          <span className="font-bold truncate">Under {thresholdVal}</span>
                          <span className="font-black text-[#248a3d]">{underItem.price.toFixed(2)}</span>
                        </button>
                      ) : <div />}
                    </div>
                  );
      ...
      {activeTab === "correct_score" && (
        <div className="flex flex-col gap-5">
          <div className="border border-gray-150 bg-gray-50/20 rounded-2xl p-4">
            <h3 className="text-[10px] font-black text-gray-700 uppercase tracking-widest mb-4 flex items-center gap-1.5">
              <span className="w-1.5 h-3.5 bg-[#34c759] rounded-full shadow-[0_0_8px_rgba(52,199,89,0.2)]"></span> Correct Score Board
            </h3>
            ...
            {correctScoreSelections.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div className="flex flex-col gap-2">
                  <h4 className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1 pb-1 border-b border-gray-200 flex items-center gap-1.5">
                    ...
                    <div className="flex flex-col gap-2">
                      {homeWins.slice(0, 8).map((cs) => (
                        ...
                        >
                          ...
                          <span className="font-black text-[#248a3d]">{cs.price.toFixed(2)}</span>
                        </button>
                      ))}
                    </div>
                  ) : (
                    <p className="text-[9px] text-gray-400 py-2 uppercase font-bold tracking-wider">No odds</p>
                  )}
                </div>
              ...
              <p className="text-[9px] text-gray-400 text-center py-4 uppercase font-black tracking-widest">No correct score odds available</p>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === "players" && (
        <div className="border border-gray-150 bg-gray-50/20 rounded-2xl p-4">
          <h3 className="text-[10px] font-black text-gray-700 uppercase tracking-widest mb-4 flex items-center gap-1.5">
            <span className="w-1.5 h-3.5 bg-[#34c759] rounded-full shadow-[0_0_8px_rgba(52,199,89,0.2)]"></span> Anytime Goalscorer
          </h3>
          {displayPlayers.length > 0 ? (
            <div className="flex flex-col gap-3">
              {displayPlayers.map((p) => {
                ...
                return (
                  <div
                    key={p.key}
                    className="flex items-center justify-between p-3 bg-white border border-gray-150 rounded-xl hover:border-[#34c759]/35 transition-colors"
                  >
                    <div className="min-w-0 mr-2 flex flex-col">
                      <div className="font-bold text-gray-800 text-xs truncate">{p.playerName}</div>
                      <span className="text-[8px] bg-gray-50 border border-gray-200 text-gray-500 font-extrabold px-2 py-0.5 rounded-full mt-1.5 inline-block uppercase tracking-wider self-start truncate max-w-full">
                    ...
                    <button
                      ...
                      className={`py-2 px-4 rounded-xl border font-black text-xs transition-all duration-300 shrink-0 ${
                        isSelected(p.key)
                          ? "border-[#34c759] bg-[#34c759]/10 text-gray-900 shadow-[0_0_10px_rgba(52,199,89,0.1)]"
                          : "border-gray-200 bg-white hover:border-[#34c759]/40 text-gray-700"
                      }`}
                    >
                      {p.price.toFixed(2)}
            ...
            <p className="text-[9px] text-gray-400 text-center py-4 uppercase font-black tracking-widest">No scorer odds available</p>
          )}
        </div>
      )}
      ...
      {activeTab === "specials" && (
        <div className="flex flex-col gap-5">
          ...
          <div className="border border-gray-150 bg-gray-50/20 rounded-2xl p-4">
            <h3 className="text-[10px] font-black text-gray-700 uppercase tracking-widest mb-4 flex items-center gap-1.5">
              <span className="w-1.5 h-3.5 bg-[#34c759] rounded-full shadow-[0_0_8px_rgba(52,199,89,0.2)]"></span> Total Corners (O/U)
            </h3>
            {cornerThresholds.length > 0 ? (
              ...
              <p className="text-[9px] text-gray-400 text-center py-2 uppercase font-black tracking-widest">No corner odds available</p>
            )}
          </div>
          ...
          <div className="border border-gray-150 bg-gray-50/20 rounded-2xl p-4">
            <h3 className="text-[10px] font-black text-gray-700 uppercase tracking-widest mb-4 flex items-center gap-1.5">
              <span className="w-1.5 h-3.5 bg-[#34c759] rounded-full shadow-[0_0_8px_rgba(52,199,89,0.2)]"></span> Both Teams to Score (BTS)
            </h3>
            {btsYesOdds > 0 ? (
              <div className="grid grid-cols-2 gap-3">
                <button
                ...
                >
                  <span className="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-1.5">YES</span>
                ...
                >
                  <span className="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-1.5">NO</span>
                  <span className="text-base font-black">{btsNoOdds.toFixed(2)}</span>
                </button>
              </div>
            ) : (
              <p className="text-[9px] text-gray-400 text-center py-2 uppercase font-black tracking-widest">No BTS odds available</p>
            )}
          </div>
        </div>
      )}
