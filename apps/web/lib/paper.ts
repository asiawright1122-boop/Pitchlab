...
export const MAX_STAKE_PER_BET = 500;
export const MIN_STAKE = 10;

export function result1x2(homeGoals: number, awayGoals: number): "H" | "D" | "A" {
  if (homeGoals > awayGoals) return "H";
  if (homeGoals < awayGoals) return "A";
  return "D";
}

export function isWinSelection(selection: string, result: "H" | "D" | "A"): boolean {
  const normSel = selection.toUpperCase();
  if (result === "H") return normSel === "H" || normSel === "1";
  if (result === "A") return normSel === "A" || normSel === "2";
  if (result === "D") return normSel === "D" || normSel === "X";
  return false;
}
...
export async function ensurePaperWallet(prisma: PrismaClient, userId: string) {
...
export async function settleOpenPaperTrades(
  prisma: PrismaClient,
  userId?: string
): Promise<{ count: number; settled: SettleResult[] }> {
  ...
  for (const t of open) {
    ...
    await prisma.$transaction(async (tx) => {
      const tradeUpdate = await tx.paperTrade.updateMany({
    ...
    if (updatedSuccessfully) {
      const tgBinding = t.user?.channelBindings?.find(b => b.channel === "telegram");
      settled.push({
        id: t.id,
        userId: t.userId,
        home: t.home,
        ...
        pnl,
        status: nextStatus,
        tgChatId: tgBinding?.externalId || null
      });
    }
  }
  return { count: n, settled };
}
